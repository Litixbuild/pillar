...
export default function PropertiesClient({
  ...
  return (
    <div className="relative min-h-screen">
      ...
      {/* Add Property modal */}
      {showModal ? (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center px-5" style={{ background: 'rgba(5,10,16,0.82)' }} onClick={(e) => { if (e.target === e.currentTarget && !isCreating) { setShowModal(false); setPropertyName(''); } }}>
          <div
            ...
            <div className="p-6">
              ...
              <div className="mt-5 space-y-1.5">
                ...
                <input
                  ...
                  onKeyDown={(e) => { if (e.key === 'Enter' && propertyName.trim()) void handleCreate(); }}
              ...
              <div className="mt-5 flex gap-2">
                <button type="button" onClick={() => void handleCreate()} disabled={!propertyName.trim() || isCreating} className="h-10 flex-1 rounded-xl text-sm font-semibold text-white transition-all duration-300 disabled:opacity-50" style={{ background: '#111111', boxShadow: '0 0 20px rgba(0,0,0,0.25)' }}>
                  {isCreating ? 'Creating…' : 'Create Property'}
                </button>
                <button type="button" onClick={() => { setShowModal(false); setPropertyName(''); }} disabled={isCreating} className="h-10 rounded-xl border px-4 text-sm font-semibold transition-all duration-200" style={dark ? { borderColor: 'rgba(255,255,255,0.07)', background: 'rgba(255,255,255,0.03)', color: 'rgba(255,255,255,0.50)' } : { borderColor: 'rgba(0,0,0,0.09)', background: 'rgba(0,0,0,0.03)', color: 'rgba(30,41,59,0.55)' }}>
                  Cancel
                </button>
              </div>
            </div>
