...
import { getHotelWindows, upsertHotelWindows } from "@/lib/hotelWindows";
...
export async function GET(req: Request, props: { params: Promise<{ slug: string }> }) {
  ...
  const windows = await getHotelWindows(slug);
...
export async function POST(req: Request, props: { params: Promise<{ slug: string }> }) {
  ...
  // Ensure every window has an id
  const windows = body.windows.map((w) => ({ ...w, id: w.id || generateWindowId() }));
  const ok = await upsertHotelWindows(slug, windows);
