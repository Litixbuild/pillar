...
export default function HotelActivityPage() {
  const params = useParams<{ slug: string }>();
  const slug   = params.slug;

  const [hotelName, setHotelName] = useState("");
  const [tab, setTab] = useState<Tab>("open");
  const [allOrders, setAllOrders]   = useState<HotelWorkOrder[]>([]);
  const [allCOs,    setAllCOs]      = useState<HotelLateCheckout[]>([]);
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const [hd, wod, lcd] = await Promise.all([
        fetch(`/api/commercial/hotels/${slug}`).then((r) => r.json()),
        fetch(`/api/commercial/hotels/${slug}/work-orders`).then((r) => r.json()),
        fetch(`/api/commercial/hotels/${slug}/late-checkout`).then((r) => r.json()),
      ]);
      setHotelName((hd as { hotel?: { name: string } }).hotel?.name ?? "");
      setAllOrders((wod as { workOrders?: HotelWorkOrder[] }).workOrders ?? []);
      setAllCOs((lcd as { checkouts?: HotelLateCheckout[] }).checkouts ?? []);
  ...
  async function resolveOrder(id: string) {
    await fetch(`/api/commercial/hotels/${slug}/work-orders/${id}/resolve`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ note: resolveNote[id] ?? "" }),
    });
    void load();
  }

  async function actionCO(id: string, action: "approved" | "denied") {
    await fetch(`/api/commercial/hotels/${slug}/late-checkout/${id}/action`, {
      method: "POST",
      headers: { "content-type": "application/json" },
  ...
  const resolvedOrders = allOrders.filter((o) => o.status === "resolved");
  const pendingCOs     = allCOs.filter((c) => c.status === "pending");
  const actionedCOs    = allCOs.filter((c) => c.status !== "pending");
              ...
              <div className="overflow-hidden rounded-2xl border border-[rgba(100,80,40,0.10)] bg-white/88 shadow-[0_4px_20px_rgba(100,80,40,0.07)] backdrop-blur-xl dark:border-white/7 dark:bg-[rgba(8,8,8,0.92)]">
                {openOrders.length === 0 ? (
                  <div className="px-6 py-12 text-center">
                    ...
                    <p className="mt-1 text-xs text-[rgba(100,80,40,0.45)] dark:text-white/35">No open work orders.</p>
                  </div>
                ) : (
                    ...
                    {openOrders.map((wo) => (
                      <div key={wo.id} className="px-6 py-5 space-y-3">
                        <div>
                          ...
                          {wo.otherMessage && <p className="mt-1 text-xs text-[rgba(100,80,40,0.55)] dark:text-white/40">{wo.otherMessage}</p>}
                        ...
                        <div className="flex gap-2">
                          ...
                          <button type="button" onClick={() => void resolveOrder(wo.id)} className="h-8 rounded-lg bg-emerald-500 px-3 text-[11px] font-semibold text-white hover:bg-emerald-600">Resolve</button>
                        </div>
                      </div>
                    ))}
                  </div>
                      ...
                      <div key={lc.id} className="px-6 py-5 space-y-3">
                        <div>
                          <div className="flex items-center gap-2 flex-wrap">
                            ...
                            <span className="text-xs text-[rgba(100,80,40,0.45)] dark:text-white/35">Expires {fmt(lc.expiresAt)}</span>
                          </div>
                          <p className="mt-1 text-[11px] text-[rgba(100,80,40,0.40)] dark:text-white/30">Requested {fmt(lc.submittedAt)}</p>
                        </div>
                        <div className="flex gap-2">
                          <button type="button" onClick={() => void actionCO(lc.id, "approved")} className="h-8 rounded-lg bg-emerald-500 px-3 text-[11px] font-semibold text-white hover:bg-emerald-600">Approve</button>
                          <button type="button" onClick={() => void actionCO(lc.id, "denied")} className="h-8 rounded-lg border border-rose-200/60 bg-rose-50/80 px-3 text-[11px] font-semibold text-rose-500 hover:bg-rose-100 dark:border-rose-500/20 dark:bg-rose-500/8 dark:text-rose-400">Deny</button>
                        </div>
                      </div>
                    ))}
                  </div>
                        ...
                        <div className="divide-y divide-[rgba(100,80,40,0.07)] dark:divide-white/5">
                          {resolvedOrders.map((wo) => (
                            <div key={wo.id} className="px-6 py-4">
                              <div className="flex items-center gap-2 flex-wrap">
                                <span className="inline-flex h-5 items-center rounded px-2 text-[10px] font-semibold text-emerald-600 bg-emerald-50 dark:bg-emerald-500/10 dark:text-emerald-400">Resolved</span>
                                <span className="text-[11px] text-[rgba(100,80,40,0.50)] dark:text-white/35">Room {wo.roomNumber} · {wo.categoryName}</span>
                              </div>
                              {wo.description && <p className="mt-1 text-xs text-slate-700 dark:text-white/65">{wo.description}</p>}
                              {wo.resolvedNote && <p className="mt-0.5 text-xs text-emerald-600/70 dark:text-emerald-400/60">Note: {wo.resolvedNote}</p>}
                        ...
                        <div className="divide-y divide-[rgba(100,80,40,0.07)] dark:divide-white/5">
                          {actionedCOs.map((lc) => (
                            <div key={lc.id} className="px-6 py-4">
                              <div className="flex items-center gap-2 flex-wrap">
                                <span className={`inline-flex h-5 items-center rounded px-2 text-[10px] font-semibold ${lc.status === "approved" ? "bg-emerald-50 text-emerald-600 dark:bg-emerald-500/10 dark:text-emerald-400" : "bg-rose-50/80 text-rose-500 dark:bg-rose-500/8 dark:text-rose-400"}`}>{lc.status === "approved" ? "Approved" : "Denied"}</span>
                                <span className="text-[11px] text-[rgba(100,80,40,0.45)] dark:text-white/35">Room {lc.roomNumber}</span>
                              </div>
                              <p className="mt-0.5 text-[10px] text-[rgba(100,80,40,0.35)] dark:text-white/25">{fmt(lc.submittedAt)}{lc.actionedAt ? ` → ${fmt(lc.actionedAt)}` : ""}</p>
                            </div>
                          ))}
                        </div>
                      </div>
