    ...
    <FeaturePageLayout>
      ...
      <section style={{ padding: 'clamp(60px, 10vw, 100px) clamp(16px, 4vw, 40px) clamp(32px, 5vw, 56px)', maxWidth: 760, margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 48 }}>
          ...
          <span style={{ width: 3, height: 3, borderRadius: '50%', background: 'rgba(255,255,255,0.20)' }} />
          <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.35)' }}>8 min read</span>
        </div>
      </section>
          ...
          {TOOLS.map((tool, i) => (
            <div key={tool.name} style={{ marginBottom: 44 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 10 }}>
                <span style={{ fontSize: 10, letterSpacing: '0.22em', textTransform: 'uppercase', color: GOLD }}>{tool.category}</span>
              </div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8 }}>
                <p style={{ fontSize: 12.5, color: 'rgba(255,255,255,0.40)', margin: 0 }}>
                  <span style={{ color: SANDY, fontWeight: 500 }}>Best for: </span>{tool.bestFor}
                </p>
                {'link' in tool && tool.link && (
                  <Link href={tool.link} style={{ fontSize: 11, color: SANDY, textDecoration: 'none', letterSpacing: '0.10em', borderBottom: '1px solid rgba(245,237,213,0.35)', paddingBottom: 1 }}>
